...
router.post(
  ...
  async (req, res) => {
    const { title, artist_id, album_id, thumbnail_url } = req.body;
    const file = req.file;
    if (!file) {
    ...
    try {
      ...
      const song = await createSong({ title, duration, file_url, artist_id, album_id, thumbnail_url });
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (!song)
      return res.status(400).send("There was an error deleting that song.");
